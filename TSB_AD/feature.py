...
class CombinedWindow:
    ...
    def convert(self, data, true_data):
        shape = (data.shape[0] - (self.window - 1), -1)
        windows = sliding_window_view(data, window_shape=self.window, axis=0).reshape(shape)[::self.stride, :].copy()
        true_windows = sliding_window_view(true_data, window_shape=self.window, axis=0).reshape(shape)[::self.stride, :].copy()
        windows[:, -1] = true_windows[:, -1]
