...
class FlowStateBankEmbeddingPAI(BaseDetector):
    ...
    def fit(self, data):
        ...
        bank_embedding, train_embeddings = get_bank_embedding(self.model, train_loader, num_cores=self.num_cores,
                                                              metric='euclidean', device=self.device, return_embeddings=True)
    ...
    def decision_function(self, data):
        ...
        scores = compute_anomaly_score(self.model, test_loader, self.bank_embedding,
                                       metric='euclidean', top_k=self.top_k, device=self.device)
